...
async def __remove_torrent(client, hash_, tag):
    """
    Deletes a torrent and its files from qBittorrent.
    """
...
async def __onDownloadError(err, tor, button=None):
    """
    Handles download errors in qBittorrent.
    """
    ext_hash = tor.hash
    ...
    LOGGER.info(f"Download error: {tor.name} - {err}")
...
async def __onSeedFinish(tor):
    """
    Handles the completion of seeding in qBittorrent.
    """
    ext_hash = tor.hash
    download = await getDownloadByGid(ext_hash[:12])
    ...
    LOGGER.info(f"Seeding finished: {tor.name}")
    listener = download.listener()
    client = download.client()
    msg = f"Seeding stopped at Ratio: {round(tor.ratio, 3)} and Time: {get_readable_time(tor.seeding_time)}"
...
async def __stop_duplicate(tor):
    """
    Checks for and stops duplicate downloads.
    """
...
async def __size_checked(tor):
    """
    Checks if the torrent size exceeds the defined limits.
    """
    download = await getDownloadByGid(tor.hash[:12])
...
async def __onDownloadComplete(tor):
    """
    Handles download completion in qBittorrent.
    """
    ext_hash = tor.hash
    tag = tor.tags
    await sleep(2)
...
async def __qb_listener():
    """
    Listens for qBittorrent events and triggers corresponding actions.
    """
    client = await sync_to_async(get_client)
    while True:
        async with qb_listener_lock:
                ...
                for tor_info in await sync_to_async(client.torrents_info):
                    ...
                    if state == "metaDL":
                        ...
                        if TORRENT_TIMEOUT and time() - tor_info.added_on >= TORRENT_TIMEOUT:
                            await __onDownloadError("Torrent is dead!", tor_info)
                        else:
                        ...
                        if config_dict['STOP_DUPLICATE'] and not QbTorrents[tag]['stop_dup_check']:
                            ...
                            await __stop_duplicate(tor_info)
                        ...
                        await __onDownloadError("Not enough space on disk!", tor_info)
                        ...
                        await __onDownloadComplete(tor_info)
                    elif state in ['pausedUP', 'pausedDL'] and QbTorrents[tag]['seeding']:
                        QbTorrents[tag]['seeding'] = False
                        await __onSeedFinish(tor_info)
            except Exception as e:
                LOGGER.error(f"qBittorrent listener error: {e}")
                client = await sync_to_async(get_client)
        await sleep(3)
...
async def onDownloadStart(tag):
    """
    Initializes a torrent in the listener.
    """
    async with qb_listener_lock:
        QbTorrents[tag] = {
            'stalled_time': time(),
