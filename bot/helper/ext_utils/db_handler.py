...
class DbManger:
    ...
    def __connect(self):
        try:
            ...
            self.__db = self.__conn[f'tellykp{bot_id}']  # Fixed: Added f-string
    ...
    async def update_private_file(self, path):
        ...
        if path == 'config__env':  # Fixed: After replace, it becomes 'config__env'
            await self.update_deploy_config()

    async def update_user_data(self, user_id):
        if self.__err:
            return
        data = user_data[user_id].copy()  # Fixed: Create a copy to avoid modifying original
        if data.get('thumb'):
            del data['thumb']
        if data.get('rclone'):
            del data['rclone']
