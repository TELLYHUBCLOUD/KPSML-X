...
def get_readable_message():
    ...
    for download in list(download_dict.values())[
        ...
        msg += BotTheme(
            "USER", User=download.message.from_user.mention(style="html")
        )
...
async def get_stats(event, key="home"):
    ...
    if key == "home":
        ...
        official_v = (
            await cmd_exec(
                "curl -o latestversion.py https://github.com/Tamilupdates/KPSML-X/raw/refs/heads/kpsmlx/bot/version.py -s && python3 latestversion.py && rm latestversion.py",
                True,
            )
        )[0]
        msg = BotTheme(
...
async def checking_access(user_id, button=None):
    ...
    if isExpired:
        token = (
            data["token"] if expire is None and "token" in data else str(uuid4())
        )
        ...
        return (
            f'<i>⚠️ Temporary Token has been expired,</i> Kindly generate a New Temp Token to start using bot Again.\n<b>⏳ Validity :</b> <code>{get_readable_time(config_dict["TOKEN_TIMEOUT"])}</code>',
            button,
        )
    return None, button
