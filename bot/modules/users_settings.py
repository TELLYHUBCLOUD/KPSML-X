...
desp_dict = {
    "rcc": [
        "♻️ RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...",
        "Send rclone.conf. \n<b>⏱️ Timeout:</b> 60 sec",
    ],
    "lprefix": [
        "🔡 Leech Filename Prefix is the Front Part attacted with the Filename of the Leech Files.",
        'Send Leech Filename Prefix. Documentation Here : <a href="https://t.me/KPSBots/87">Click Me</a> \n<b>⏱️ Timeout:</b> 60 sec',
    ],
    "lsuffix": [
        "🔤 Leech Filename Suffix is the End Part attached with the Filename of the Leech Files",
        'Send Leech Filename Suffix. Documentation Here : <a href="https://t.me/KPSBots/87">Click Me</a> \n<b>⏱️ Timeout:</b> 60 sec',
    ],
    "lremname": [
        "✂️ Leech Filename Remname is combination of Regex(s) used for removing or manipulating Filename of the Leech Files",
        'Send Leech Filename Remname. Documentation Here : <a href="https://t.me/KPSBots/87">Click Me</a> \n<b>⏱️ Timeout:</b> 60 sec',
    ],
    "lcaption": [
        "📝 Leech Caption is the Custom Caption on the Leech Files Uploaded by the bot",
        'Send Leech Caption. You can add HTML tags. Documentation Here : <a href="https://t.me/KPSBots/87">Click Me</a> \n<b>⏱️ Timeout:</b> 60 sec',
    ],
    "ldump": [
        "🗑️ Leech Files User Dump for Personal Use as a Storage.",
        "Send Leech Dump Channel ID\n➲ <b>Format:</b> \ntitle chat_id/@username\ntitle2 chat_id2/@username2. \n\n<b>⚠️ NOTE:</b>Make Bot Admin in the Channel else it will not accept\n<b>⏱️ Timeout:</b> 60 sec",
    ],
    "metadata": [
        "ℹ️ Your channel name that should be used while editing metadata of the file",
        "Send File Metadata\n<b>⏱️ Timeout:</b> 60 sec",
    ],
    "lattachment": [
        "📎 Attachment url, it will added in mkv as thumbnail or cover photo, whetever you say.",
        "Send Telegraph photo url\n\n<b>⏱️ Timeout:</b> 60 sec",
    ],
    "mprefix": [
        "🅰️ Mirror Filename Prefix is the Front Part attacted with the Filename of the Mirrored/Cloned Files.",
        "Send Mirror Filename Prefix. \n<b>⏱️ Timeout:</b> 60 sec",
    ],
    "msuffix": [
        "🅱️ Mirror Filename Suffix is the End Part attached with the Filename of the Mirrored/Cloned Files",
        "Send Mirror Filename Suffix. \n<b>⏱️ Timeout:</b> 60 sec",
    ],
    "mremname": [
        "✂️ Mirror Filename Remname is combination of Regex(s) used for removing or manipulating Filename of the Mirrored/Cloned Files",
        "Send Mirror Filename Remname. \n<b>⏱️ Timeout:</b> 60 sec",
    ],
    "thumb": [
        "🖼️ Custom Thumbnail to appear on the Leeched files uploaded by the bot",
    ...
    "usess": [
        f'🔐 User Session is Telegram Session used to Download Private Contents from Private Channels with no compromise in Privacy, Build with Encryption.\n{"<b>⚠️ Warning:</b> This Bot is not secured. We recommend asking the group owner to set the Upstream repo to the Official repo. If it is not the official repo, then KPSML-X is not responsible for any issues that may occur in your account." if config_dict["UPSTREAM_REPO"] != "https://github.com/Tamilupdates/KPSML-X" else "✅ Bot is Secure. You can use the session securely."}',
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        bot_pm = (
            "Enabled"
            if user_dict.get("bot_pm", config_dict["BOT_PM"])
            else "Disabled"
        )
        buttons.ibutton(
        ...
        save_mode = (
            "Save As Dump" if user_dict.get("save_mode") else "Save As BotPM"
        )
        buttons.ibutton(
        ...
        lattachment = (
            "Not Exists"
            if (val := user_dict.get("lattachment", config_dict.get("ATTACHMENT", "")))
            == ""
            else val
        )
        buttons.ibutton(
            f"{'✅️' if lattachment != 'Not Exists' else ''} Leech Attachment",
            f"userset {user_id} lattachment",
        )

        metadata = (
            "Not Exists"
            if (val := user_dict.get("metadata", config_dict.get("METADATA", ""))) == ""
            else val
        )
        buttons.ibutton(
            f"{'✅️' if metadata != 'Not Exists' else ''} Leech Metadata",
            f"userset {user_id} metadata",
        )
        ...
        if key == "rcc":
            ...
            text += (
                f"➲ <b>RClone.Conf File :</b> <i>{'' if set_exist else 'Not'} Exists</i>\n\n"
            )
        elif key == "thumb":
            set_exist = await aiopath.exists(thumbpath)
            text += (
                f"➲ <b>Custom Thumbnail :</b> <i>{'' if set_exist else 'Not'} Exists</i>\n\n"
            )
        elif key == "yt_opt":
            set_exist = (
                "Not Exists"
        ...
        if (
            ...
            if key == "thumb":
                buttons.ibutton(
                    "View Thumbnail", f"userset {user_id} vthumb", "header"
                )
            elif key == "user_tds":
                buttons.ibutton(
                    "Show UserTDs", f"userset {user_id} show_tds", "header"
                )
        ...
        if set_arg and (reply_to := message.reply_to_message):
            if message.from_user.id != reply_to.from_user.id:
                return await editMessage(
                    msg, "<i>Reply to Your Own Message for Setting via Args Directly</i>"
                )
            if (
                set_arg
                in [
...
➲ <b>🔤 Leech Filename Suffix :</b>
    /cmd -s lsuffix
➲ <b>✂️ Leech Filename Remname :</b>
    /cmd -s lremname
➲ <b>📝 Leech Filename Caption :</b>
    /cmd -s lcaption
➲ <b>ℹ️ Leech Filename Metadata :</b>
    /cmd -s metadata
➲ <b>📎 Leech Filename Attachment :</b>
    /cmd -s lattachment
➲ <b>📺 YT-DLP Options :</b>
    /cmd -s yt_opt
➲ <b>🗑️ Leech User Dump :</b>
    /cmd -s ldump""",
        )
    else:
...
async def set_custom(client, message, pre_event, key, direct=False):
    ...
    if key in ["gofile", "streamtape"]:
        ...
        for dump_item in value.split("\n"):
            ...
            dump_info = dump_item.rsplit(
                maxsplit=(
                    1 if dump_item.split()[-1].startswith(("-100", "@")) else 0
                )
            )
    ...
    await update_user_settings(
        pre_event, key, return_key, msg=message, sdirect=direct
    )
    if DATABASE_URL:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        await sendMessage(
            message, from_user.mention, buttons.build_menu(1), thumb_path
        )
        ...
        await query.answer()
        update_user_ldata(
            user_id, data[2][1:], {} if data[2] == "duser_tds" else ""
        )
        if data[2] == "duser_tds":
...
async def send_users_settings(client, message):
    ...
    if not userid:
        ...
        msg = (
            f'{(await user_info(userid)).mention(style="html")} ( <code>{userid}</code> ):'
        )
