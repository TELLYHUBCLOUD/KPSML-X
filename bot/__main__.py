...
async def log(_, message):
    buttons = ButtonMaker()
    buttons.ibutton(
        BotTheme("LOG_DISPLAY_BT"), f"kpsmlx {message.from_user.id} logdisplay"
    )
    buttons.ibutton(
        BotTheme("WEB_PASTE_BT"), f"kpsmlx {message.from_user.id} webpaste"
    )
    ...
    try:
        ...
        for query in query_list:
            ...
            for page in range(1, total_pages + 1):
                ...
                if len(images) == 0:
                    LOGGER.info(
                        "⚠️ Maybe Site is Blocked on your Server, Add Images Manually !!"
                    )
                for img in images:
        ...
        for chat_id in config_dict["LEECH_LOG_ID"].split():
            ...
            try:
                ...
                if chat.type == ChatType.CHANNEL:
                    ...
                    if (
                        not (await chat.get_member(bot.me.id)).status
                    ...
                    if (
                        user
                        and not (await chat.get_member(user.me.id)).status
                        in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR]
                    ):
                        LOGGER.error(
...
async def main():
    ...
    LOGGER.info(f"🤖 TellY-Mirror Bot [@{bot_name}] Started!")
    if user:
        LOGGER.info(f"👤 TellY-Mirror User [@{user.me.username}] Ready!")
